    ...
    def __init__(
        ...
        try:
            self.model = torch.hub.load(
                dinov3_repo_path, dinov3_model_type, source="local", weights=dinov3_weights_path
            )
            self.model.eval()
            ...
            _logger.info(
                "[Teacher Model] Successfully loaded DINOv3 teacher from local repo and weights."
            )
        ...
        _logger.info(
            f"[Teacher Model] DINOv3 initialized. Feature dimension: {self.teacher_feature_dim}."
        )
        _logger.info(
    ...
    def forward(self, images: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            if patch_tokens.dim() != 3:
                ...
                raise ValueError(
                    "DINOv3 model's output 'x_norm_patchtokens' is not in expected 3D format."
                )
            ...
            _logger.info(f"[Teacher Model] Spatial size: {teacher_feature_map.shape[2:]}")
